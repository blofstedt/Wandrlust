...
/*
 * Pure geometry helpers (simplification, area math, water/tiny-part trimming)
 * live in boundaryGeometry.ts now — split out purely to shrink this file, with
 * no change in behavior. See that file's header for the exact boundary: this
 * file keeps everything with caching, network I/O, or the ingest pipeline.
 */
import {
  approxAreaSqKm, MAX_PARTS_DETAIL, MAX_PARTS_MERGED, MAX_PARTS_FOR_MERGE,
  visiblePartDeg2, prunedFeatures, withoutTinyHolesIn, withoutWater,
  generaliseGeometry, orientToLonLat, esriRingsToGeoJson
