...
import {
  MARK_VIEWBOX, MARK_EXTENT, MARK_LIT, MARK_SHADOW, MARK_INK, BRAND_GRADIENT_STOPS
 ...
 * `shared/brandMark.mjs` for the shape and why it is the shape it is. The
 * header used to draw a lucide `<Compass>` in a gradient square, which was a
 * DIFFERENT drawing that happened to be roughly the same idea: change one and
 * the other quietly stopped matching. Now there is one mark with two renderers.
 *
 ...
 * paints the same thing, and the alternative is a fresh gradient in the DOM for
 * every badge.
 */
export const BrandMark: React.FC<{
  ...
  /* The rose sits at 74% of the tile, matching `icon-192.png`. Reckoned
     against the mark's own widest points rather than its 24-unit design box,
     which has margin in it that would otherwise be counted twice. */
  const box = MARK_VIEWBOX / 0.74 / MARK_EXTENT;
  ...
  return (
    ...
    >
      ...
      <path
        d={MARK_LIT}
        fill={MARK_INK.lit} stroke={MARK_INK.lit}
        strokeWidth={0.45} strokeLinejoin="round"
      />
      <path
        d={MARK_SHADOW}
        fill={MARK_INK.shadow} stroke={MARK_INK.shadow}
        strokeWidth={0.45} strokeLinejoin="round"
      />
    </svg>
  );
