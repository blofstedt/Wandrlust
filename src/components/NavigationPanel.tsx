...
export const NavigationPanel: React.FC<NavigationPanelProps> = ({
  destination, route, weather, coverage, camperCount, friendCount, onExit, onRecentre
}) => {
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 z-[1400] p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pointer-events-none">
      <div className="mx-auto max-w-2xl space-y-2">
        {/*
          The one thing that must never be missed: this is a line on a map,
          not a co-pilot. Kept above the numbers so it is read first.
        */}
        {critical.length > 0 && (
        ...
        <div className="pointer-events-auto rounded-2xl bg-slate-900/95 backdrop-blur-md border border-slate-700/70 shadow-2xl overflow-hidden anim-in-up">
          <div className="flex items-stretch">
            ...
            <div className="flex flex-col border-l border-slate-700/70">
              <button
                onClick={onRecentre}
                className="flex-1 px-3.5 text-slate-300 hover:text-white hover:bg-slate-800/70 flex items-center justify-center"
