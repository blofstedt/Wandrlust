 ...
 *
 * The formula itself lives in `shared/geoMath.ts` so the server bundle can
 * use the identical calculation — see that file for why.
 */
import { haversineMiles, haversineKm } from '../../shared/geoMath';

const toRad = (deg: number): number => (deg * Math.PI) / 180;

export const distanceMiles = haversineMiles;
