 ...
 * sizes.
 *
 * WHAT IT IS NOT: a store. Nothing here survives a reload or a cold start,
 * and nothing here is a source of truth. Callers decide what is worth
 * remembering — and in this app that never includes a failure, because a
 * cached "we could not check" would keep telling a camper there is nothing
 * there long after the network came back.
 *
 * LIVES IN shared/ because both bundles used to hand-roll this exact
 * get/set/expiry/evict-oldest pattern independently — at least ten separate
 * copies across server/*.ts, each one a chance for the eviction or TTL logic
 * to quietly drift from the others. One implementation, imported by both.
 */
export class TtlCache<T> {
  ...
  /** How many live entries are currently held, expired or not. */
  get size(): number {
